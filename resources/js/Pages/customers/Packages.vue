        ...
        <div class="bg-white rounded-md shadow overflow-x-auto">
            <table class="w-full whitespace-nowrap">
                ...
                >
                    ...
                    <td class="border-t">
                        <Link class="flex items-center px-6 py-4" tabindex="-1">
                            <span v-if="url === '/customer/received/history'">{{
                                packageData.ltime_recv_end
                                    ? packageData.ltime_recv_end
                                    : "No Date"
                            }}</span>
                            <span v-if="url === '/customer/viewed/history'">{{
                                packageData.time_post_opened
                            }}</span>
                            <span v-if="url === '/customer/sent/history'">{{
                                packageData.ltime_send_end
                                    ? packageData.ltime_send_end
                                    : "No Date"
                            }}</span>

                            <span v-if="url === '/customer/received/today'">{{
                                packageData.ltime_recv_end
                            }}</span>
                            <span v-if="url === '/customer/sent/today'">{{
                                packageData.ltime_send_end
                            }}</span>
                            <span v-if="url === '/customer/viewed/today'">{{
                                packageData.time_post_opened
                            }}</span>
                        </Link>
                    </td>
